...
CVXOPT 1.3.3, SDPA (sdpa-python 0.2.3), QTQP 0.0.7 (MKL Pardiso and cuDSS backends; record only,
...
|---|---|---|---|
| `qp` | Maros-Meszaros (138) and QPLIB continuous convex (19) | every solver at nominal 1e-4, 1e-5 and 1e-6 (QTQP also 1e-8) | 300 s |
...
| `mittelmann` | Hans Mittelmann's LP benchmark set (37) | 1e-4 and 1e-5 (QTQP 1e-4, 1e-6, 1e-8) | 1800 s |
...
| `infeas` | Netlib infeasible LPs (29), SDPLIB infeasible SDPs (4) | page settings plus SCS / OSQP / Clarabel / PDLP variants with matched infeasibility tolerances | 300 s / 900 s |

The site's plots use the runs matched by achieved accuracy (interior-point solvers from their 1e-6 runs,
Clarabel 1e-8 on SDP, PDLP and ProxQP from 1e-5), verification at 10x the plot's target, failures charged
